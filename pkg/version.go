...
func CheckAvailableNewVersion() (bool, string, *semver.Version) {
	...
	if err != nil {
		return false, fmt.Sprintln("Error while trying to get the current version, mostlikely current version `%s` is not a valid semver string, error: `%s`", Version, err), nil
	}
